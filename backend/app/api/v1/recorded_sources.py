...
@router.get("/recorded-source/{source_key}/preview")
def recorded_preview(source_key: str, _user: CurrentUser = Depends(get_current_user)) -> dict:
    if not settings.RECORDED_SOURCE_SNAPSHOT_DIR:
        raise HTTPException(status_code=404, detail="No recorded source is configured")
    try:
        return recorded_timetable_preview(
            Path(settings.RECORDED_SOURCE_SNAPSHOT_DIR),
            settings.RECORDED_SOURCE_MANIFEST_SHA256,
            source_key,
        )
    except LookupError as exc:
        raise HTTPException(status_code=404, detail="Unknown recorded feed") from exc
    except (OSError, ValueError) as exc:
        raise HTTPException(
            status_code=503, detail="Recorded timetable could not be verified or decoded"
        ) from exc
